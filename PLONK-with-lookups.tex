...
\newcommand{\papertitle}{A simplified polynomial protocol for look-up tables}
...
\begin{document}
    ...
    \begin{abstract}
     We present a protocol for checking the values of a comitted polynomial $f\in \polysofdeg{n}$ over a multiplicative subgroup $H\subset \F$ of size $n$, are contained in the values of a different comitted polynomial $t\in \polysofdeg{d}$ over the same subgroup $H$. Our protocol can be viewed as a simplification of one from Bootle et. al \cite{arya} for a similar problem, with potential efficiency improvements. In particular, \cite{arya}'s protocol requires comitting to several auxiliary polynomials of degree $d\cdot \log n$, whereas ours requires only one commitment to a single auxiliary polynomial of degree $d+n$, which is much smaller in the case $d\sim n$.
     
     One common use case of this primitive in the zk-SNARK setting is a ``batched range proof'', where one wishes to check all of $f$'s values on $H$ are in a range $[0,\ldots,M-1]$. We present a slightly optimized protocol for this special case, and pose improving it as an open problem.
    \end{abstract}
...
We use the ranged polynomial protocol terminology from \cite{plonk} to describe our protocols.
Here a prover sends polynomials of a certain degree bound $d$,
and at the end of protocol, the verifier can ask whether certain identities hold between the polynomials sent during the protocol, the input polynomials, and the preprocessed polynomials, on a predefined set $H$.
It is shown in \cite{plonk} how such a protocol can be compiled into one in the algebraic group model using the polynomial commitment scheme of \cite{kate}.


For such a protocol \prot, let $\aggdeg{\prot}$ be the maximum over $s_1,\ldots,s_t$ sent by an honest prover during protocol execution of the sum of the degrees of the $s_i$ plus the maximal degree of the verifier identity when plugging in $s_1,\ldots,s_t$ into the verifier identity minus the size of $H$.
...
\end{enumerate}
 ...
 \begin{enumerate}
  ...
  \item 
  \[(X-\hgen^{n+1})Z(X)(1+\beta)\cdot (\gamma + f(X))(\gamma(1+\beta)+ t(X)+\beta t(X\cdot \hgen) )\]
  \[\equiv (X-\hgen^{n+1}) Z(X\cdot \hgen)(\gamma(1+\beta)+ s(X)+\beta s(X\cdot \hgen))(\gamma(1+\beta)+ s(\cosetgen \cdot X)+\beta s(\cosetgen \cdot X\cdot \hgen)).\]
  \item $L_{n+1}(s(X)-s(\cosetgen X \hgen))\equiv 0$
...
\section{An optimized solution for continuous ranges}
Suppose we wish to check that $f_i \subset \set{0,\ldots,d-1}$ for some integer $d< n$.
We could use our above protocol while setting $t_i = i-1$ for $i\in [d]$.
By using e.g. $d=n$, the above protocol allowes us to check $f\subset \set{0,\ldots,n-1}$ using one auxiliary polynomial of degree $2n$ (the polynomial $s$ representing the sorted witness) and another of degree $n+1$ (the product aggregating polynomial $Z$). 
We show a variant of the protocol specifically for continuous ranges with the same prover efficiency allowing for the larger range \set{0,\ldots,c\cdot n-1}.
