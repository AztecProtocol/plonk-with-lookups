...
We show that this check can be done efficiently using a ``grand product argument'' similar to the one used in \cite{plonk}'s permutation argument.
...
We will (ab)use the notation $f\subset t$ to mean $\sett{f_i}{i\in [n]}\subset \sett{t_i}{i\in [d]}$. Let $\dom=\set{\hgen,\ldots,\hgen^{n}}$ be a subset of a multiplicative subgroup of order $n+1$.  Denote by $\cosetgen$ be an element of a non-trivial coset of $H$ in $\F^*$.
...
Call $P'\subset [n+d-1]$ the set of these $d-1$ indices $j$.
 ...
 Claim \ref{clm:main_lookup} motivates the following protocol. We use the ranged polynomial protocol terminology from \cite{plonk} to describe it.
 
It will be conveient to assume $d=n+1$. (If $d\leq n$ pad $t$ with $n-d+1$ repetitions of the last element.)
 \paragraph{Preprocessed polynomials:}
The polynomial $t \in \polysofdeg{n}$  describing the lookup values.


\paragraph{Inputs: $f\in \polysofdeg{n}$}
\paragraph{Protocol:}
\begin{enumerate}
 \item Let $s\in \F^{2n+1}$ be the vector that is $(f,t)$ sorted by $t$. Also denote by $s$
 the following polynomial in $\polysofdeg{2n+2}$ that represents $s$ on $\dom \cup \cosetgen \dom$:
$s(\hgen^i) = s_i$ for $i\in [n+1]$; and $s(\cosetgen\hgen ^i)= s_{n+i}$ for each $i\in [n+1]$.
 
 \item \prvpoly computes the polynomial $s$ and sends it to \ideal.
...
\end{enumerate}
 ...
 \begin{enumerate}
  ...
  \item $Z(X)(1+\beta)\cdot (\gamma + f(X))(\gamma(1+\beta)+ t(X)+\beta t(X\cdot \hgen) ) \equiv  Z(X\cdot \hgen)(\gamma(1+\beta)+ s(X)+\beta s(X\cdot \hgen))(\gamma(1+\beta)+ s(\cosetgen \cdot X)+\beta s(\cosetgen \cdot X\cdot \hgen))$.
  \item $L_{1}(s(\cosetgen X\cdot \hgen)-s(X/\hgen))\equiv 0$
%   \item $Z(\cosetgen\cdot  X)(\gamma(1+\beta)+ t(X)+\beta t(X\cdot \hgen) )=Z(\cosetgen\hgen \cdot  X)(\gamma(1+\beta)+ s(\cosetgen X)+\beta s(\cosetgen X\cdot \hgen) )$.
  \item $L_{n}(Z(X\cdot \hgen)-1)\equiv 0$
